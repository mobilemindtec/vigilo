...
object validators:
  ...
  def validateNumber(fieldName: String, value: VNumber, validations: Seq[validation])
                    (using msg: ValidatorMessages): Either[Seq[String], Unit] =
    def createStringMsg(key: String, args: Any*) =
      msg.message(
        s"validation.number.$key",
        msg.message(fieldName) +: args *
      )
            ...
            .foldRight(Seq.empty[String]) { (v, acc) =>
              v match
                ...
                case date(min = min, max = null, pattern = pattern) if min != null =>
                  ...
                  then acc :+ msg.message(
                    "validation.date.min", msg.message(fieldName), DateTimeFormatter.ofPattern(pattern).format(min))
                  else acc
                case date(min = null, max = max, pattern = pattern) if max != null =>
                  if value.isAfter(max)
                  then acc :+ msg.message(
                    "validation.date.max", msg.message(fieldName), DateTimeFormatter.ofPattern(pattern).format(max))
                  else acc
