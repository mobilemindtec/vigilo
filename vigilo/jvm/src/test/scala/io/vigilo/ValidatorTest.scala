...
case class RegexpValidationModel(
                                  @regexp(pattern = "\\d{3}-\\d{3}-\\d{4}") phoneNumber: String = ""
                                ) derives Validator

case class RelationValidationModel(
...
class ValidationTests extends AnyFlatSpec {
  ...
  "Number validation" should "fail for values below minimum" in {
    val model = NumberValidationModel(
      minValue = 5,
      maxValue = 50,
      rangeValue = 30
    )
  ...
  it should "fail for values outside range" in {
    // Below minimum
    val modelBelow = NumberValidationModel(
      minValue = 15,
      maxValue = 50,
      rangeValue = 15
    )
    ...
    // Above maximum
    val modelAbove = NumberValidationModel(
      minValue = 15,
      maxValue = 50,
      rangeValue = 60
    )
